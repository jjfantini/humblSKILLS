...
**Naming convention for humblSKILLS:** `[<verb>-]smart-<noun>`.

- `smart-` is the namespace marker. Every first-party skill carries it.
- Prefix a verb **only when it carries information** — `analyze-`, `create-`.
  Do not prefix `use-`: every skill is used, so the word distinguishes nothing.
- Ported skills keep their upstream namespace instead (e.g. the `better-*`
  family), so provenance stays legible at a glance.

```
smart-animation                  # no verb needed
create-smart-scroll-animation    # verb distinguishes it from the reference skill
analyze-smart-system-design      # verb is the whole point of the skill
better-typography                # ported - upstream namespace, not ours
```
