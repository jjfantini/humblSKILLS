# Decisions

Reasoning memory. Each entry records a non-obvious choice: the context,
the options considered, what was chosen, why, and the observed result.
Never delete entries - if a decision is reversed, add a new entry that
references the old one.

Entry shape:

```
### <YYYY-MM-DD> | <short title>
- Context: <the situation that required a choice>
- Options: (A) <opt>, (B) <opt>, (C) <opt>
- Chose: <letter and name>
- Why: <the rationale, ideally citing evidence>
- Result: <what happened after, or "TBD">
```

---

### 2026-04-17 | Add `compatibility` frontmatter, skip `allowed-tools`
- Context: agentskills.io spec defines 4 optional SKILL.md frontmatter fields (`license`, `compatibility`, `metadata`, `allowed-tools`). Needed to decide which to propagate through this skill and its scaffold.
- Options: (A) add both `compatibility` and `allowed-tools`, (B) add `compatibility` only, (C) add neither and only document in a wiki concept.
- Chose: B - add `compatibility` to this skill's SKILL.md, emit a loud TODO placeholder in scaffold.sh, and propagate through workflow/validation docs. Skip `allowed-tools` entirely.
- Why: `compatibility` carries real signal (this skill has bash scripts, consumers need to know). Max 500 chars, cheap, per-spec appropriate. `allowed-tools` is flagged experimental by the spec itself, agent support varies, and this is a meta-skill with open-ended tool use - pre-approving a list would either be decorative (too permissive) or restrictive (breaks the skill). Scaffold emits a TODO-or-delete line (not silent omit) so new-skill authors consciously decide instead of forgetting the field exists.
- Result: SKILL.md now declares bash + POSIX requirements. Scaffold forces explicit decision. Future skill authors see the field in the new `wiki/smart/spec/skill-frontmatter.md` concept.

### 2026-04-19 | New `anthropic/` wiki context (alongside existing `smart/`)
- Context: Anthropic published "The Complete Guide to Building Skills for Claude" PDF. Needed to ingest best practices so every scaffolded humblSKILL inherits them. Existing `smart/spec/skill-frontmatter.md` already distills the agentskills.io spec.
- Options: (A) merge new material into `smart/` context as new categories, (B) create a dedicated `anthropic/` context with categories `frontmatter`, `description`, `structure`, `testing`, `patterns`, `troubleshooting`, (C) overwrite existing `smart/spec/skill-frontmatter.md` with distilled PDF content.
- Chose: B - dedicated `anthropic/` context.
- Why: (1) attribution is first-class - the `context:` folder name carries provenance back to Anthropic's guide vs agentskills.io. (2) When Anthropic ships a new version of the PDF we update every concept under `anthropic/*` and know exactly which pages to re-read. (3) `smart/spec/skill-frontmatter.md` (agentskills source) and `anthropic/frontmatter/requirements.md` (Anthropic source) can legitimately coexist as complementary views on the same underlying schema. Lint's contradiction heuristic will flag duplicate `concept:` values across contexts for human audit - acceptable trade-off.
- Result: 8 new concepts under `anthropic/` (frontmatter/requirements, frontmatter/security, description/trigger-design, structure/progressive-disclosure, structure/file-layout, testing/three-layer-approach, patterns/five-patterns, troubleshooting/common-failures). All cite `references/raw/anthropic-skill-building-guide.pdf`. SKILL.md `How to Use` now routes Anthropic-sourced questions into `anthropic/<category>/` concepts.

### 2026-04-19 | Reverse 2026-04-17 decision: include `allowed-tools` after all
- Context: Anthropic's official guide (Reference B) and Chapter 5 "Instructions not followed" both surface `allowed-tools` as a real tool that improves security posture and reduces ambiguity. The 2026-04-17 rationale was "experimental, agent support varies". The PDF (Oct 2025 vintage) lists it as a first-class optional field with a clear syntax. Reality moved faster than the 2026-04-17 decision.
- Options: (A) keep skipping `allowed-tools` to honor prior decision, (B) include `allowed-tools` with an explicit value in this skill + TODO-or-delete in scaffold template, (C) include but leave as comment-only placeholder.
- Chose: B - declare `allowed-tools: "Bash(bash:*) Bash(sh:*) Read Write Edit Glob Grep"` in this skill's SKILL.md, emit TODO-or-delete in scaffold template so new-skill authors decide explicitly.
- Why: field is now canonical in Anthropic's guide, not experimental. Meta-skill can declare a tight set because its own tool usage is predictable (bash scripts + file ops). Scaffold template stays permissive (TODO-or-delete) because per-skill tool needs vary.
- Result: supersedes the "skip `allowed-tools`" part of the 2026-04-17 decision. `compatibility` decision from 2026-04-17 remains in force.

### 2026-04-19 | Move humblSKILLS extension fields from top-level to `metadata:`
- Context: Anthropic's spec (Reference B) treats top-level frontmatter as `name`/`description`/`license`/`compatibility`/`allowed-tools` + a free-form `metadata:` map for everything else. humblSKILLS had been putting `version`, `tags`, `platforms`, `requires`, `preserve` at the top level, which would be rejected as non-standard by strict validators.
- Options: (A) keep top-level (remain non-compliant but historical), (B) move all humblSKILLS fields under `metadata:` with a hard break, (C) move to `metadata:` with a soft-transition fallback (parser reads metadata first, falls back to top-level, warns).
- Chose: C - soft transition.
- Why: keeps the in-repo migration risk-free (existing skills keep loading during the cutover), surfaces deprecation warnings on every registry build so external consumers learn the new shape, and preserves a rollback path if a downstream consumer breaks. Remove the fallback in a later release once no warnings appear.
- Result: `cli/internal/frontmatter/Frontmatter` now exposes `Version()`, `Requires()`, `Platforms()`, `Tags()`, `Preserve()` accessor methods that fall back to legacy top-level fields. `DeprecationWarnings()` surfaces remaining top-level usage. All 5 in-repo skills migrated to the new shape; `build-registry` output is warning-free.

### 2026-08-12 | Keep `disable-model-invocation: true` on a registry-distributed skill
- Context: the upstream flat `orchestrate` skill set `disable-model-invocation: true`, so the model can never auto-trigger it. No other skill in the humblSKILLS registry uses that field, and the smart-skill validation checklist emphasises trigger phrases in `description:`.
- Options: (A) drop the field so the skill auto-triggers like every other registry skill, (B) keep it and rely on explicit `/smart-orchestrate` invocation, (C) keep it but omit trigger phrases from the description since they can never fire automatically.
- Chose: B - keep the field, and still write a full trigger-phrase description.
- Why: a full orchestration run spends frontier tokens, creates git worktrees, and dispatches subagents across CLIs. Auto-triggering that on a vague "orchestrate this" is a real cost and blast-radius event the user did not opt into, which is exactly why the upstream author set the flag. Trigger phrases stay in the description because they are what the skill picker and `humblskills search` match on - they are not wasted just because auto-invocation is off. The CLI's frontmatter parser accepts unknown top-level keys, so the field passes `build-registry` untouched.
- Result: TBD - watch whether registry users report "installed it and nothing happened". If so, the fix is a README note, not dropping the flag.

### 2026-08-12 | Rename the raw source to `orchestrate-SKILL.md`
- Context: the brain spec says the LLM never renames files in `references/raw/`. The migration source was literally named `SKILL.md`, which would land at `references/raw/SKILL.md`.
- Options: (A) keep the name verbatim per the raw-territory rule, (B) rename to `orchestrate-SKILL.md`.
- Chose: B.
- Why: a second file named `SKILL.md` inside a skill directory is a genuine hazard for humans grepping the tree, and would be a hazard for tooling too if `build-registry` ever walked recursively (today it only joins `skills/<name>/SKILL.md`, so it is safe by one implementation detail). The no-rename rule exists to protect human-dropped provenance; here the drop was performed by the migration itself, and the prefix adds provenance rather than erasing it.
- Result: raw file is `references/raw/orchestrate-SKILL.md`, cited by all 9 concepts, byte-identical to the upstream flat skill.

### 2026-08-12 | Refresh model names in the wiki, leave `references/raw/` at 4.5
- Context: Grok 4.5 was superseded by Grok 4.6. Two wiki concepts named 4.5 as a model tier (`roles/parent-orchestrator` for the Cursor parent slot, `roles/worker-agent` for the cheap worker slot), and so does the preserved upstream skill at `references/raw/orchestrate-SKILL.md`.
- Options: (A) update the wiki only, (B) update the wiki and the raw file so nothing in the skill reads "4.5", (C) update the wiki and annotate the raw file's line as superseded.
- Chose: A - wiki only. The raw file keeps saying Grok 4.5.
- Why: raw is human territory and immutable by the brain spec ("Never edit raw to satisfy a wiki claim"). It is a point-in-time snapshot of what was handed over, and it doubles as the provenance baseline every concept's `sources:` array points at - editing it to match a later fact destroys what makes the citation meaningful, and silently rewrites history for anyone diffing the distillation against its origin. A wiki that has moved ahead of its raw source is the normal state of a living skill, not drift to be reconciled.
- Result: `roles/parent-orchestrator.md` and `roles/worker-agent.md` say Grok 4.6; the raw file still says 4.5. Read the difference as "wiki is current, snapshot is historical" - that is exactly what `sources:` is for. Skill version 1.0.0 -> 1.0.1.

### 2026-08-13 | Author a raw file as measurement provenance instead of leaving `sources:` empty
- Context: the new `routing/cursor-cli-models` concept is built from measurements taken in-session (3-trial model sweeps), not distilled from anything a human dropped into `references/raw/`. Lint permits an empty `sources:` (soft warning, "synthesis-only"), and the brain spec is explicit that `references/raw/` is human territory and immutable - see the 2026-08-12 entry that refused to update the raw file's Grok version.
- Options: (A) leave `sources:` empty and accept the orphan-concept warning, (B) author `references/raw/cursor-cli-model-sweep-2026-08-13.md` holding the verbatim sweep output and cite it, (C) inline every raw number into the wiki concept and cite nothing.
- Chose: B.
- Why: the numbers ARE the evidence, and a routing table without its measurement log is an assertion. A future session that wants to re-measure needs the exact invocation, trial count, control design, and incident context - that is a provenance record, which is precisely what `references/raw/` is for. (A) discards the evidence; (C) buries it in prose and makes the concept unreadable.
- Note the important asymmetry with 2026-08-12: there, raw was a historical snapshot and the wiki was allowed to move ahead of it. Here raw is live evidence, so the two must stay in sync - when a PENDING row gets measured, patch BOTH files. Do not let this precedent be read as "the LLM may now rewrite handover material in raw"; it may only author its own measurement logs.
- Result: 10 wiki concepts, 2 raw files, lint clean.

### 2026-08-19 | Reverse 2026-08-12 decision: drop `disable-model-invocation: true`
- Context: the 2026-08-12 entry kept the upstream flag so only an explicit `/smart-orchestrate` could start a run, reasoning that auto-triggering a frontier-token, worktree-creating run on a vague "orchestrate this" was a cost the user had not opted into. User has since asked directly for this skill to be model-invocable.
- Options: (A) keep the field per the 2026-08-12 rationale, (B) drop the field so the description's trigger phrases can auto-invoke like every other registry skill, (C) drop the field but weaken the trigger phrases in the description to reduce accidental auto-fires.
- Chose: B - drop `disable-model-invocation: true` entirely, keep the description's trigger phrases and negative triggers as-is.
- Why: explicit user instruction overrides the prior default-safe posture; the field was this skill's only deviation from the rest of the registry, and the description already carries a "Do NOT use for a single-file edit..." negative trigger that does the restraint work the flag used to do structurally. Weakening the trigger phrases (C) would fight the goal of making the skill actually reachable.
- Result: skill version 1.1.0 -> 1.2.0. Watch for over-eager auto-invocation on ambiguous requests; if that surfaces, the fix is sharper negative triggers in the description, not re-adding the flag.

### 2026-09-10 | Author fetched vendor docs into `references/raw/` rather than citing URLs
- Context: adding Fable 5.1 and GPT-6 Astra prompting guidance needs a `sources:` array. The material is Anthropic's and OpenAI's live documentation, not a file a human dropped in. The brain spec says raw is human territory and the LLM never edits or renames it; the 2026-08-13 entry permits the LLM to author its OWN measurement logs there.
- Options: (A) leave `sources:` empty and accept the orphan-concept warning, (B) cite the doc URLs directly in `sources:` even though lint requires paths under `references/raw/`, (C) fetch each doc as markdown into `references/raw/` with a dated filename and cite that.
- Chose: C. Files: `anthropic-prompting-claude-fable-5-1-2026-09-10.md` (platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1), `anthropic-claude-prompting-best-practices-2026-09-10.md` (same path, claude-prompting-best-practices), `openai-gpt-6-astra-latest-model-2026-09-10.md` (developers.openai.com/api/docs/guides/latest-model).
- Why: (B) is not permitted by lint - `sources:` paths must resolve under `references/raw/`. (A) discards the provenance that makes a verbatim prompt block trustworthy, and these concepts are almost entirely quotation: without the snapshot a reader cannot tell a quote from a paraphrase. Vendor docs are also mutable and unversioned - the URL will describe a different model in six months, so a URL alone is a citation that silently rots. The date is in the filename precisely so a future session can diff against the live page.
- Note the boundary this does NOT cross: the files are byte-identical to what the vendor served, fetched with `curl`, never edited. This extends the 2026-08-13 precedent (the LLM may author its own measurement logs) to "the LLM may deposit an unmodified external snapshot." It still may not rewrite handover material a human placed there.
- Result: 3 new raw docs + 1 new measurement log (`orchestrator-model-sweep-2026-09-10.md`), cited by 3 new concepts and 7 updated ones. Lint clean.

### 2026-09-10 | One new `prompting` category, not parallel `anthropic/` and `openai/` contexts
- Context: this repo has precedent both ways. The 2026-04-19 decision in `smart-skill` created a dedicated `anthropic/` context so the folder name carries provenance. Here the material is two vendors' prompting guides.
- Options: (A) mirror the smart-skill precedent with `anthropic/` and `openai/` contexts, (B) one `orchestrate/prompting/` category holding both vendors' concepts, (C) fold the guidance into the existing `routing/` and `contracts/` concepts with no new files.
- Chose: B.
- Why: (A) buys provenance this skill already has - every concept's `sources:` names the exact vendor snapshot, and `_index.md` regenerates the reverse map, so the folder name would be redundant signal at the cost of a two-context taxonomy for three files. It would also split the single most useful artefact here, the *comparison*: the two vendors' defaults point in opposite directions on delegation, and that fact has no home in a taxonomy that separates them by vendor. (C) was rejected because a verbatim prompt block per vendor is a distinct concept, and burying it inside `model-selection.md` would push that file past the one-concept-per-file rule.
- Result: `orchestrate/prompting/` with 3 concepts - `anthropic-fable-5-1`, `openai-gpt-6-astra`, `brief-prompting-by-vendor`. The cross-vendor comparison table lives in the third, which is where a dispatching parent actually needs it.

### 2026-09-10 | Retire the Grok 4.6 / Composer 2.5 worker recommendation and the Cursor `auto` parent
- Context: `roles/worker-agent.md` listed "Grok 4.6, Composer 2.5" as preferred cheap worker models and `roles/parent-orchestrator.md` listed "Auto Intelligence, or Grok 4.6 High" as the Cursor parent. Both predate the 2026-08-13 sweeps, which measure Grok 4.6 at ~3/24 across all eight tiers, `composer-2.5` at 1/4, and `auto` at 0/12. The skill was recommending backends it separately proves broken.
- Options: (A) leave the role files alone since the routing concept already carries the measurements, (B) correct the role files to the measured-good IDs and say explicitly that the old advice is retired, (C) delete the model lists from the role files entirely and point at the routing concept.
- Chose: B.
- Why: (A) is untenable - the role concepts are what a parent reads first, and progressive disclosure means many runs will read them and never open the routing table, so the stale list is what actually gets dispatched. (C) is cleaner in the abstract but strips the role file of the one-line answer ("what do I dispatch to") that makes it useful standalone, and a pointer-only file invites the same drift back. Saying "an earlier version of this concept recommended both" is deliberate: a reader who remembers the old advice needs to know it was withdrawn on evidence, not silently reworded.
- Result: worker slots are now a measured table (`gpt-5.3-codex-low-fast` 21/21, `gpt-5.6-luna-high`, `claude-opus-5-thinking-high`), the Cursor parent is `claude-opus-5-thinking-high`, and both role files carry the retirement note. Two new anti-patterns cover the general form.

### 2026-09-10 | A gate is a commit boundary, not a blocking boundary
- Context: Anthropic's Fable 5.1 guide states that on coding tasks, letting the lead agent continue while subagents run lowers average time to completion at similar quality and cost, and recommends a dispatch tool that returns immediately. `loop/session-loop.md` step 8 says the parent commits a gate before opening the next one. Pasted side by side these read as contradictory advice about waiting.
- Options: (A) adopt the non-blocking guidance and soften step 8, (B) keep step 8 and omit the non-blocking guidance, (C) keep both and name the distinction that reconciles them.
- Chose: C.
- Why: they constrain different things and the conflict is only apparent. Non-blocking dispatch is about *concurrency within* a gate - the parent may dispatch worker B and keep reading code while worker A runs. Step 8 is about *sequencing across* gates - gate 2 may not open until gate 1 is verified and committed, because the commit is the baseline the next brief is written against. (A) would delete the only mechanism that makes a late phase's regression attributable; (B) would leave measured wall-clock savings on the table and let a future reader think the skill hadn't read the vendor guidance.
- Result: `loop/session-loop.md` gains a two-row table separating "may overlap" from "must not overlap". Named here because the next session to read Anthropic's guidance will feel the same tension and should not re-litigate it.
