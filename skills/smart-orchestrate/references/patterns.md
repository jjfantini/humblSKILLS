...
### 2026-09-10 | Frontier orchestrator sweep: GPT-6 Astra reachable on Codex, Fable 5.1 entitlement-blocked on Cursor
- Context: add Claude Fable 5.1 and GPT-6 Astra to this skill as parent/director models. Both are newer than every existing entry, so nothing in the wiki could say whether either is reachable from the CLIs this skill dispatches through.
- Approach: same protocol as the 2026-08-13 sweep - explicit `--model`, 3 trials per arm, `gpt-5.3-codex-low-fast` as the control at BOTH ends. Arms: `claude-fable-5-1-{high,thinking-high,max}` on cursor-agent 2026.08.25, `gpt-6-astra` on codex-cli 0.154.0. Inventory checked first with `--list-models` (227 IDs) and `claude --help`.
- Result: control 6/6 at 5-14s (now 21/21 lifetime, so no incident drift). All three Fable 5.1 arms 0/3, failing in 3-13s with `ActionRequiredError: Model Blocked  Please ask your admin to enable access to Claude Fable 5.` A follow-up probe of `claude-fable-5-high` returned the identical message, so the block is family-wide and account-level, not a 5.1 regression. `gpt-6-astra` on codex: 4/4 at 6-7s. GPT-6 Astra appears in 0 of the 227 Cursor IDs. `claude -p --model claude-fable-5-1` on Claude Code 2.1.267: 4/4 at 6-7s, and the `fable` alias also resolves.
- Worked: the bracketed control again. Nine consecutive failures between two 3/3 control blocks is what makes "blocked" a conclusion instead of "something was down at 11:50." Also: checking the inventory BEFORE pinging - `--list-models | grep gpt-6` returning nothing saved three pointless arms and is itself the finding that Astra is Codex-only.
- Didn't: `--list-models` misled again, in a NEW way. In August it over-reported IDs the API rejects as invalid (`gpt-5.4-nano-*`); here it over-reported ten `claude-fable-5-1-*` IDs that are perfectly valid and simply not enabled for this account. The second kind is more dangerous - the ID is real and correctly spelled, so nothing looks wrong until dispatch.
- Also: availability is per-CLI, not per-model. Fable 5.1 was 0/9 on cursor-agent and 4/4 on Claude Code within the same hour on the same machine, because the two authenticate against different accounts.
- Lesson: there is a FIFTH failure class - `ActionRequiredError: Model Blocked`, ~3-13s, never retryable, and unlike the other four it is ENVIRONMENT-SPECIFIC: the same ID may be 3/3 on another account whose admin enabled it. So an availability table has two expiry mechanisms, vendor uptime AND your own org's entitlements - `claude-fable-5-medium` was 3/3 on 2026-08-13 and blocked on 2026-09-10 with the model unchanged. Route GPT-6 Astra through `codex -m gpt-6-astra`, never Cursor. And treat `(NO ZDR)` in a Cursor model label as a routing constraint in its own right: Fable 5.1 carries 30-day retention and no ZDR without express Anthropic authorization, which vetoes it for proprietary code regardless of score.
