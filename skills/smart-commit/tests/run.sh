#!/usr/bin/env bash
# tests/run.sh - opt-in test suite for smart-commit scripts.
#
...
expect_contains "footer on by default" 'Authored by humblSKILLS; "smart-commit"' "$out"
...
expect_contains "real commit body has footer" 'Authored by humblSKILLS; "smart-commit"' "$body"
