...
**Exception — the humblSKILLS authorship footer.** The line `Authored by humblSKILLS; "smart-commit"` is the skill's own marker (see the *Authorship footer* section in `SKILL.md`). It is on by default, attributes authorship to the skill rather than to a model, and can be disabled by the user per-conversation or persistently. It is not an AI attribution line and the rule above does not apply to it.
